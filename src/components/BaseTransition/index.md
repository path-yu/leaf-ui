...
```tsx
import React, { useState } from 'react';
import { CollapseTransition, Switch,Space } from 'leaf-ui';
export default () => {
...
```tsx
import React, { useState } from 'react';
import { Zoom, Switch ,Space} from 'leaf-ui';
export default () => {
  const [checked, setChecked] = useState(true);
  ...
  return (
    <>
      ...
      <Space>
        ...
        <Zoom value={checked} style={{ transitionDelay: checked ? '500ms' : '0ms',display:"inline-block" }}>
          <div style={{ width: '80px', height: '80px', background: '#ccc' ,}}></div>
        </Zoom>
      </Space>
    </>
